...
- BOMBO/kick: 60-120Hz cuerpo + 2-5k click. s("bd..").bank("RolandTR909"|"RolandTR808"). techno→909 con shape(0.3-0.5); trap/phonk→808 largo. Al centro y mono; algo de shape para que pegue en altavoces pequeños. SIDECHAIN REAL: para que el kick AGACHE al sub/bajo, el bombo emite .duck(2) (agacha el orbit 2) y la rama grave va con .orbit(2).
...
- SUB/808: 20-60Hz. note("c1..").s("sine") o 808 afinado a la TÓNICA del tema, lpf(200-500). Mono, UNO solo en la zona sub. SLIDE del 808 (trap/phonk/basshall/EBM): note("f#1 ~ ~ f#1").s("sine").penv(7).pdecay(0.15) → el 808 ENTRA deslizándose hasta la nota (penv=semitonos de la envolvente de pitch, pdecay=tiempo del glide); NO uses .slide (es no-op). Sidechain al bombo enrutando el sub con .orbit(2) (recibe el .duck(2) del bombo). Dancehall→sub enorme y simple con silencios.
...
- PITCH / AFINACIÓN: se hacen con note()/n() (re-afinan sintes Y samples), NO existe un método .pitch(). El SLIDE del 808 es la envolvente de pitch penv/pdecay (NO .slide, que es un stub no-op en superdough). Sidechain REAL: .duck(K) en el bombo + .orbit(K) en la rama que debe agacharse. Nada de glissando/portamento/reverb/filter (para reverb usa room; para filtrar lpf/hpf/bpf).`;
