...
4. ~~**P0.3** (variantes por sección).~~ **✅ HECHO en su forma núcleo** (`278afb4` + evoluciones): pestañas de sección en el secuenciador; **rejilla universal** también para melodías (`61360e8`, preferencia explícita del usuario — piano roll queda de vista alterna); secciones en silencio **sembradas** con la instrumentación (`ecd7ee7`); **audición de sección** — ▶ toca la sección editada en loop al instante, punto "sonando" en pestañas, playhead honesto (`ea67727`). Pendiente ampliado: SongTimeline ↔ compases del arrange (hoy: pestañas muestran compases; edición en "entradas"/código).
5. ~~**P1.1, P1.2, P1.5, P1.6**.~~ **✅ HECHOS**: mono-bajo (`61dade8` — highpass 120 Hz en la rama side del M/S), micro-timing por paso + velocity continua + humanize centrado (`0a9e075` — sub-filas vel/timing en el panel ≋, `.late("…")` paralelo con negativos verificados en krill), grabador por **AudioWorklet** con fallback (`cb620e4` — el WAV final sin cracks de UI; freeze incluido). Quedan los **P2** (escenas↔máster/EQ, banco por pista UI, trigger de canción cuantizado, analyze condicional, freeze >16 ciclos, salud frontend).
